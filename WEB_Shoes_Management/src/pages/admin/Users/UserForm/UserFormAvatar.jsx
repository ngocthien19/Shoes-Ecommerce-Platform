...
import { FiCamera, FiTrash2, FiUploadCloud, FiUser } from 'react-icons/fi'

export const UserFormAvatar = ({ avatarPreview, onAvatarChange, onRemoveAvatar, isEditMode }) => {
  ...
  const getAvatarUrl = () => {
    if (avatarPreview) {
      if (typeof avatarPreview === 'object' && avatarPreview.secure_url) {
        return avatarPreview.secure_url
      }
    ...
    >
      <h3 className="text-lg font-black text-gray-900 flex items-center gap-2 border-b border-gray-100 pb-3 mb-4">
        <div className="w-8 h-8 rounded-xl bg-emerald-500/10 flex items-center justify-center">
          <FiCamera className="text-emerald-500" size={16} />
        </div>
        Ảnh đại diện
      </h3>
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="relative group">
          <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-emerald-100 shadow-lg bg-gray-100">
            {avatarUrl ? (
              <img
                src={avatarUrl}
                alt="Avatar"
