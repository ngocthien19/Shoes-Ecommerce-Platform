...
export const TabProfile = ({ user, loading, onUpdateProfile }) => {
  const { register, handleSubmit, setValue, formState: { errors } } = useForm()

  // Đổ dữ liệu ban đầu từ Redux vào form
  useEffect(() => {
    if (user) {
  ...
  return (
    <form onSubmit={handleSubmit(onUpdateProfile)} className="pt-8 space-y-6 w-full animate-fadeIn">
      ...
      {/* Validate Số điện thoại khớp logic Backend */}
      <InputField
        label="Số điện thoại"
        type="text"
      ...
      <InputField
        ...
        icon={FiMapPin}
        {...register('address', {
          required: 'Địa chỉ giao hàng mặc định là thông tin bắt buộc.',
          minLength: { value: 10, message: 'Vui lòng nhập địa chỉ cụ thể và rõ ràng hơn.' }
        })}
