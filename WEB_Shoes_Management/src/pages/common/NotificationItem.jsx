    ...
    >
      ...
      <div className="flex gap-3 relative z-10">
        <motion.div
          ...
          className={`shrink-0 w-11 h-11 rounded-xl ${bg} flex items-center justify-center shadow-sm transition-all duration-300 group-hover:shadow-md`}
        >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <h4 className={`text-sm font-bold truncate ${isUnread ? 'text-gray-900' : 'text-gray-600'}`}>
              {notification.title}
            </h4>
          ...
          <p className={`text-xs mt-1 line-clamp-2 ${isUnread ? 'text-gray-600' : 'text-gray-500'}`}>
            {contentData.message}
          </p>
          <div className="flex items-center gap-2 mt-2">
            {!isUnread && (
              <div className="flex items-center gap-1 font-bold">
                ...
                <span className="text-[10px] text-green-500 font-medium">Đã đọc</span>
              </div>
            )}
            {isUnread && (
              <div className="flex items-center gap-1">
                <div className="w-1.5 h-1.5 rounded-full bg-brand-primary animate-pulse" />
                <span className="text-[9px] text-brand-primary font-medium">Chưa đọc</span>
              </div>
            )}
          </div>
        </div>
